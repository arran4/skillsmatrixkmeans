...
func NewRoot(name, version, commit, date string) (*RootCmd, error) {
	...
	c.StringVar(&c.inputFile, "file", "", "Path to the input CSV file")

	c.IntVar(&c.k, "k", 3, "Number of clusters")

	c.StringVar(&c.outputFile, "out", "", "Path to the output JSON file optional")

	c.CommandAction = func(c *RootCmd) error {

		err := cli.Run(c.inputFile, c.k, c.outputFile)
		if err != nil {
			...
			return fmt.Errorf("skills-kmeans failed: %w", err)
		}
		return nil
	}
